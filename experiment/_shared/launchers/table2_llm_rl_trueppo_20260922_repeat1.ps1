<#
Table 2 LLM-RL repeat_1 lazy calibration/train launcher.
...
This launcher deliberately binds no world-model/reward-predictor CLI inputs:
LLM-RL decisions are frozen-prior K6/H4 only, while PPO still trains on the
Full-Reward real transition channel.
#>
[CmdletBinding()]
param(
...
$Target = Join-Path $MainProjectRoot 'outputs\formal_v3\table2\llm_rl_trueppo_20260922_repeat1\repeat_1'
$LogRoot = Join-Path $MainProjectRoot 'outputs\launcher_logs\table2_llm_rl_trueppo_20260922_repeat1'
...
$ExpectedHead = '669859819cda21119f2b1fb5906613b917c3d6b8'
...
function Assert-WorktreeSnapshot {
    ...
    if ((git -C $Worktree rev-parse HEAD).Trim() -ne $ExpectedHead) { throw 'BLOCKED: LLM-RL worktree HEAD drift' }
}
...
function Assert-NoTargetWriter {
    ...
    $writers = @(Get-CimInstance -ClassName Win32_Process -ErrorAction Stop | Where-Object {
        ...
        ($cmd -match '(?i)run_table23_true_ppo' -and $cmd -match '(?i)--variant\s+llm_rl' -and $cmd -match '(?i)--row-id\s+llm_rl')
    })
...
function Assert-FrozenAssets {
    ...
    Require-Sha $Prior $ExpectedSha.Prior 'frozen train prior' | Out-Null
    Require-Sha $PriorCacheManifest $ExpectedSha.PriorCacheManifest 'prior cache manifest' | Out-Null
    Require-Sha $PriorCacheBinding $ExpectedSha.PriorCacheBinding 'prior cache binding' | Out-Null
    Require-Sha $WorldModel $ExpectedSha.WorldModel 'world model audit asset' | Out-Null
    Require-Sha $RewardModel $ExpectedSha.RewardModel 'Full-Reward predictor audit asset' | Out-Null
    ...
    Require-Contains $Runtime 'FormalPPOVariant.LLM_RL, True, False, 6, "frozen_teacher_K6_H4"' 'LLM-RL variant identity'
    Require-Contains $Runtime 'self.spec.uses_world_model' 'world-model decision gate'
    Require-Contains $Runtime 'build_posterior_candidate_features(state, plans, preferences, rollout)' 'candidate feature contract'
    Require-Contains $Runtime 'prior.load_with_audit' 'offline prior lookup'
}
function Get-ContractEvidence([object]$Resources) {
    return [ordered]@{
        schema = 'cc4_v3_table2_llm_rl_launcher_plan_v1'; status = 'PASS'; stage = $Stage
        variant = $Variant; table_id = 'table2'; row_id = $RowId; repeat_index = 1; policy_seed = 51001
        reward_mode = 'Full-Reward'; reward_channel = 'real_response_reward';
        candidate_semantics = 'frozen_teacher_K6_H4'; candidate_count = 6; horizon = 4
        uses_llm_prior = $true; uses_world_model = $false; wm_return_or_uncertainty_in_decision = $false
        online_provider_allowed = $false; provider_calls = 0; cache_only = $true
        ...
        prior_sha256 = $ExpectedSha.Prior; prior_cache_manifest_sha256 = $ExpectedSha.PriorCacheManifest; prior_cache_binding_sha256 = $ExpectedSha.PriorCacheBinding
...
function Get-RunnerArgs([string]$RunStage) {
    $args = @('-B', '-m', 'formal_experiments.ours.run_table23_true_ppo', '--project-root', $ProjectRoot,
        ...
        '--policy-seed', '51001', '--prior-artifact', $Prior, '--prior-cache-manifest', $PriorCacheManifest,
        '--prior-cache-binding', $PriorCacheBinding, '--replay-manifest', $ReplayManifest,
        '--replay-train-jsonl', $ReplayTrain, '--replay-validation-jsonl', $ReplayValidation,
        '--table-id', 'table2', '--row-id', $RowId, '--source', 'final_reward_20260917',
        '--canonical-run-id', 'llm_rl_trueppo_20260922_repeat1', '--device', 'cuda', '--out', $Target)
...
function Assert-CalibrationTarget {
    ...
    if ($lockPayload.schema -ne 'cc4_v3_true_ppo_writer_lock_v1') {
        throw 'BLOCKED: writer lock schema is not the frozen true-PPO schema'
    }
    if ($lockPayload.status -eq 'ACTIVE') {
        $identityMatches = ($lockPayload.stage -eq 'train' -and $lockPayload.variant -eq $Variant -and
            $lockPayload.reward_mode -eq 'Full-Reward' -and [int]$lockPayload.policy_seed -eq 51001)
        if (-not $identityMatches) { throw 'BLOCKED: stale ACTIVE writer lock identity does not match this train target' }
        $liveWriter = Get-Process -Id ([int]$lockPayload.pid) -ErrorAction SilentlyContinue
        if ($null -ne $liveWriter) { throw "BLOCKED: calibration/train target has a live ACTIVE writer PID $($lockPayload.pid)" }
        Write-Host "RECOVERY_GATE: preserving dead ACTIVE/train lock evidence for runner reclaim (PID $($lockPayload.pid))"
    }
    elseif ($lockPayload.status -ne 'RELEASED') {
        throw "BLOCKED: calibration/train lock is neither RELEASED nor a reclaimable dead ACTIVE/train lock ($($lockPayload.status))"
    }
...
if ($Stage -eq 'calibration') {
    ...
    if ($DryRun) {
        ...
        $plan.runner_args = Get-RunnerArgs 'calibration'
        $plan.cli_help_checked = $true
        Assert-CliHelp
