...
def rollout_model_env(
    ...
    for action in plan:
        next_obs, reward, done, _ = model_env.step(
            np.tile(action, (num_samples, 1)), sample=sample
        )
...
def rollout_agent_trajectories(
    ...
    Args:
        ...
        condition (callable, optional): a function to determine whether to add transition to replay
            buffer.
    ...
    while True:
        ...
        while not done:
            if replay_buffer is not None:
                action, next_obs, reward, done, info = step_env_and_add_to_buffer(
                    env,
                    obs,
                    agent,
                    agent_kwargs,
